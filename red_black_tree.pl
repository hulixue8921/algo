...
sub Insert {
    ...
    #  say "xxxx" if &tree_key($node) eq 6;
    ...
    if ($tree) {
        ...
        # say "xxxx". &tree_key($p) if &tree_key($node) eq 6;
...
sub Balance {
    ...
    my $b_l = $location->[-1] eq 'right' ? 'left' : 'right';
    my $B   = $parents->[-1]->{$b_l};
    my $P   = $parents->[-1];
    my $L   = $location->[-1];
    ...
    if ( $color eq 'r' ) {
        ...
        if ( $B->{c} eq 'r' ) {
            ###brother  red, so : brother's son must black, parents must black. i is  b
            ###action : left rolation or right rolation , and brother change to b
            $B->{c} = 'b';
            if ( $b_l eq 'right' ) {
                &L_rolation( $B, $P, $parents->[-2], $location->[-2] );
            }
            elsif ( $b_l eq 'left' ) {
                &R_rolation( $B, $P, $parents->[-2], $location->[-2] );
            }
        }
        elsif ( $B->{c} eq 'b' ) {
            ####find a red node (this node must is Brother'son)
            while (@$parents) {
                my $p   = $parents->[-1];
                my $l   = $location->[-1];
                my $b_l = $l eq 'right' ? 'left' : 'right';
                my $b   = $p->{$b_l};
                ...
                if ( $b->{$b_l}->{c} eq 'r' ) {
                    ...
                    if ( $b_l eq 'right' ) {
                        ...
                        $p->{right}->{c} = $p->{c};
                        $p->{c} = 'b';
                        &L_rolation( $p->{right}, $p, $parents->[-2],
                            $location->[-2] );
                    }
                    elsif ( $b_l eq 'left' ) {
...
#&Insert($Tree ,{ 3=>'xxx',right => undef , left=>undef } ,[],[]);
#&Insert($Tree ,{ 11=>'xxx',right => undef , left=>undef } ,[],[]);
#&Insert($Tree ,{ 4=>'xxx',right => undef , left=>undef } ,[],[]);
